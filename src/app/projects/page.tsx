  ...
  return (
    <>
      ...
      {projects.length ? (
        ...
        <div className="panel grid min-h-64 place-items-center p-8 text-center"><div><FolderSearch className="mx-auto text-[#6b7970]" size={26} /><p className="mt-4 text-[12px] font-semibold">No hay proyectos</p><p className="mt-1 text-[9px] text-[#6f7c74]">{session.role === "admin" ? "Crea tu primer proyecto desde la página de Resumen." : "Tu administrador todavía no te ha asignado una participación."}</p></div></div>
      )}
    </>
  );
}
