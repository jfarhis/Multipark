...
export default async function InvestorDashboardPage() {
  ...
  return (
    <>
      ...
      <section className="mt-7"><div className="mb-4"><p className="text-[14px] font-semibold">Tus participaciones</p><p className="mt-1 text-[10px] text-[#7d8a82]">El rendimiento se calcula con tu porcentaje en cada proyecto.</p></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{positions.map(({ project, stake }) => <ProjectCard key={project.id} project={project} data={data} stake={stake} />)}</div>{positions.length === 0 ? <div className="panel p-10 text-center text-[10px] text-[#7d8a82]">Todavía no tienes participaciones asignadas.</div> : null}</section>
    </>
  );
}
