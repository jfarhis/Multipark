  ...
  return (
    <>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.45fr_1fr]">
        <article className="panel p-5"><p className="text-[13px] font-semibold">Tus distribuciones</p><p className="mt-1 text-[10px] text-[#6b7594]">Pagos por mes y acumulado de todas tus participaciones</p><div className="mt-5 h-[235px]">{monthlySeries.length ? <DistributionLineChart values={monthlySeries.map((item) => item.amount)} labels={monthlySeries.map((item) => item.label)} cumulative={monthlySeries.map((item) => item.cumulative)} /> : <div className="grid h-full place-items-center text-[10px] text-[#6b7594]">Todavía no hay distribuciones.</div>}</div></article>
        <article className="panel flex flex-col justify-between overflow-hidden p-5"><div><p className="eyebrow">Última distribución</p><p className="tabular mt-4 text-[32px] font-semibold tracking-[-0.04em]">{compactMoney.format(latestDistribution?.amount ?? 0)}</p><p className="mt-1 text-[11px] text-[#7c86a6]">{latestDistribution ? `Registrada el ${latestDistribution.date}` : "Todavía no hay distribuciones"}</p></div><div className="mt-8 rounded-xl bg-[#161d33] p-4"><div className="flex items-center justify-between text-[10px]"><span className="text-[#8a93b2]">Capital devuelto a la fecha</span><span className="font-semibold text-[#2dd4a7]">{returnedPct.toFixed(1)}%</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#0b101e]"><div className="h-full rounded-full bg-[linear-gradient(90deg,#5b8cff,#2dd4a7)]" style={{ width: `${returnedPct}%` }} /></div></div></article>
      </section>
      {positions.length >= 2 ? (
        <section className="mt-4 grid gap-4 xl:grid-cols-2">
          <article className="panel p-5"><p className="text-[13px] font-semibold">Comparativa de tus proyectos</p><p className="mt-1 text-[10px] text-[#6b7594]">Avance, ocupación y TIR proyectada lado a lado</p><div className="mt-4 h-[280px]"><ProjectRadarChart projects={positions.map(({ project }) => ({ label: project.name, construction: project.constructionPct, occupancy: project.occupancyPct, irr: project.projectedIrr }))} /></div></article>
          <article className="panel overflow-hidden"><div className="border-b border-[#1b2340] p-5"><p className="text-[13px] font-semibold">Resumen por proyecto</p><p className="mt-1 text-[10px] text-[#6b7594]">Tu posición en cada proyecto</p></div><div className="divide-y divide-[#1b2340]">{positions.map(({ project, stake }) => <div key={project.id} className="table-row flex items-center justify-between gap-3 px-5 py-3.5"><div className="min-w-0"><p className="truncate text-[11px] font-medium">{project.name}</p><p className="mt-1 text-[9px] text-[#596382]">Participación {stake.stakePct}%</p></div><p className="tabular shrink-0 text-[11px] font-semibold text-[#7ea2ff]">{compactMoney.format(stake.capitalCommitted)}</p></div>)}</div></article>
        </section>
      ) : null}
      <section className="mt-7"><div className="mb-4"><p className="text-[14px] font-semibold">Tus participaciones</p><p className="mt-1 text-[10px] text-[#7c86a6]">El rendimiento se calcula con tu porcentaje en cada proyecto.</p></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{positions.map(({ project, stake }) => <ProjectCard key={project.id} project={project} data={data} stake={stake} />)}</div>{positions.length === 0 ? <div className="panel p-10 text-center text-[10px] text-[#7c86a6]">Todavía no tienes participaciones asignadas.</div> : null}</section>
    </>
  );
}
