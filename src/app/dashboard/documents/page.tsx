...
export default async function InvestorDocumentsPage() {
  ...
  const visible = data.documents.filter((document) => projectIds.has(document.projectId) && (document.investorId === null || document.investorId === investorId)); const rows = visible.map((document) => ({ id: document.id, title: document.title, projectId: document.projectId, project: data.projects.find((project) => project.id === document.projectId)?.name ?? "Project", investorId: document.investorId, investor: document.investorId ? "Private to you" : "Project-wide", type: document.type, date: document.uploadedDate, url: document.fileUrl }));
