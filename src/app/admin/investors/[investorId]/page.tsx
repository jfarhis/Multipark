...
export default async function InvestorDetailPage({ params }: PageProps<"/admin/investors/[investorId]">) {
  ...
  return <><Link href="/admin/investors" className="mb-5 inline-flex items-center gap-1.5 text-[10px] text-[#b18f7e]"><ArrowLeft size={12} />Volver a inversionistas</Link><div className="mb-6 flex items-center gap-4"><span className="grid size-12 place-items-center rounded-2xl bg-[linear-gradient(135deg,#f2571a,#ffc46b)] text-[12px] font-bold">{investor.name.split(" ").map((word) => word[0]).join("")}</span><div><h1 className="text-[22px] font-medium">{investor.name}</h1><p className="mt-1 text-[10px] text-[#a3806f]">{investor.email} · Referencia bancaria: {investor.bankDetails}</p></div></div><InvestorProfileForm investor={investor} /><section className="mb-4 grid gap-3 sm:grid-cols-3"><KpiCard label="Total invertido" value={money.format(portfolio.totalInvested)} icon={Landmark} /><KpiCard label="Total distribuido" value={money.format(portfolio.totalDistributed)} icon={WalletCards} /><KpiCard label="Participaciones" value={String(portfolio.stakes.length)} icon={Building2} /></section><div className="grid gap-4 xl:grid-cols-[1.3fr_1fr]"><InvestorEditor investor={investor} data={data} /><section className="panel overflow-hidden"><div className="border-b border-[#35201a] p-5"><p className="text-[13px] font-semibold">Historial de pagos</p><p className="mt-1 text-[10px] text-[#a3806f]">Todas las distribuciones recibidas</p></div><div className="max-h-[420px] divide-y divide-[#35201a]/70 overflow-auto">{history.map((item) => <div key={item.id} className="table-row flex items-center justify-between gap-3 px-5 py-3.5"><div><p className="text-[10px] font-medium">{data.projects.find((project) => project.id === item.projectId)?.name}</p><p className="mt-1 text-[9px] text-[#82604e]">{item.date}</p></div><p className="tabular text-[11px] font-semibold text-[#ffc46b]">{money.format(item.amount)}</p></div>)}{history.length === 0 ? <div className="p-10 text-center text-[10px] text-[#a3806f]">Todavía no hay pagos.</div> : null}</div></section></div></>;
}
