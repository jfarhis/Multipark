...
export default async function InvestorsPage() {
  ...
  return <><PageHeading eyebrow="Socios de capital" title="Inversionistas e inversiones" description="Primero agrega al inversionista. Después abre su ficha y registra cuánto invirtió en MXN en cada proyecto." action={<div className="flex gap-2"><ExportButton filename="gasfar-inversionistas.csv" rows={rows.map((row) => ({ Nombre: row.name, Correo: row.email, Participaciones: row.projects, Invertido_MXN: row.invested, Distribuido_MXN: row.distributed }))} /><a href="#invite-investor" className="primary-button px-3"><UserPlus size={14} />Agregar inversionista</a></div>} /><InvestorTable rows={rows} /><InvestorInviteForm /></>;
}
