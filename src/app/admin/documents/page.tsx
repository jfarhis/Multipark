...
export default async function AdminDocumentsPage() {
  ...
  return <><PageHeading eyebrow="Bóveda privada" title="Documentos y comprobantes" description="Carga, edita, reemplaza o elimina archivos y controla exactamente quién puede verlos." action={<div className="flex items-center gap-2 rounded-xl border border-white/8 bg-[#221110] px-3 py-2 text-[10px] text-[#b18f7e]"><LockKeyhole size={13} className="text-[#ffc46b]" />Almacenamiento privado conectado</div>} /><DocumentUploadForm data={data} /><DocumentTable rows={rows} data={data} editable /></>;
}
