...
export default async function AdminDocumentsPage() {
  ...
  return <><PageHeading eyebrow="Bóveda privada" title="Documentos y comprobantes" description="Carga, edita, reemplaza o elimina archivos y controla exactamente quién puede verlos." action={<div className="flex items-center gap-2 rounded-xl border border-white/8 bg-[#131a30] px-3 py-2 text-[10px] text-[#8a93b2]"><LockKeyhole size={13} className="text-[#2dd4a7]" />Almacenamiento privado conectado</div>} /><DocumentUploadForm data={data} /><DocumentTable rows={rows} data={data} editable /></>;
}
