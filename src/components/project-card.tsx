...
export function ProjectCard({ project, data, stake }: { project: Project; data: DashboardData; stake?: InvestorProjectStake }) {
  ...
  return (
    <Link href={`/projects/${project.id}`} className="panel panel-hover group block p-4">
      ...
      <div className="mt-5"><div className="mb-2 flex items-center justify-between text-[9px]"><span className="text-[#7c8981]">Construction progress</span><span className="tabular font-semibold">{project.constructionPct}%</span></div><ProgressBar value={project.constructionPct} color={project.status === "delayed" ? "#ff5470" : "#8b72ff"} /></div>
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-[#202b25] pt-4"><div><p className="text-[8px] text-[#69766e]">{stake ? "Your stake" : "Raised"}</p><p className="tabular mt-1 text-[10px] font-semibold">{stake ? `${stake.stakePct}%` : compactMoney.format(capital)}</p></div><div><p className="text-[8px] text-[#69766e]">{stake ? "Your capital" : "Distributed"}</p><p className="tabular mt-1 text-[10px] font-semibold">{compactMoney.format(stake ? capital : distributed)}</p></div><div><p className="text-[8px] text-[#69766e]">Projected IRR</p><p className="tabular mt-1 text-[10px] font-semibold text-[#35d99a]">{project.projectedIrr}%</p></div></div>
      <p className="mt-3 flex items-center gap-1.5 text-[8px] text-[#5f6c64]"><CalendarDays size={9} />Estimated completion {completionLabel(project)}</p>
    </Link>
  );
}
