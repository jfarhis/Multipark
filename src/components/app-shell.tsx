...
import { Building2, Camera, FileText, HandCoins, LayoutDashboard, PanelLeftClose, PanelLeftOpen, Plus, Search, Settings, Users } from "lucide-react";
    ...
    <div className="min-h-screen">
      <aside className={`fixed inset-y-0 left-0 z-40 hidden border-r border-[#1b2340] bg-[#0b101e] transition-[width] duration-200 md:flex md:flex-col ${collapsed ? "w-[68px]" : "w-[216px]"}`}>
        ...
        <div className={collapsed ? "px-3 pt-4" : "px-3 pt-5"}>
          {!collapsed && <p className="px-3 text-[12px] font-semibold uppercase tracking-[.16em] text-[#3e4762]">Espacio de trabajo</p>}
          <nav className="mt-2 space-y-1">
            {nav.map(([label, href, Icon]) => {
              const active = isActive(pathname, href);
              return (
                <Link key={label} href={href} title={label} className={`flex h-10 items-center rounded-lg text-[14px] font-medium transition ${collapsed ? "justify-center" : "gap-3 px-3"} ${active ? "bg-[#1b2547] text-white shadow-[0_0_18px_rgba(77,124,254,.15)]" : "text-[#6b7594] hover:bg-[#121830] hover:text-white"}`}>
        ...
        <div className={`mt-auto mb-3 ${collapsed ? "px-3" : "mx-3"}`}>
          <button onClick={toggle} title={collapsed ? "Expandir menú" : "Colapsar menú"} aria-label={collapsed ? "Expandir menú" : "Colapsar menú"} className={`mb-2 flex h-9 w-full items-center rounded-lg text-[13px] font-medium text-[#6b7594] transition hover:bg-[#121830] hover:text-white ${collapsed ? "justify-center" : "gap-3 px-3"}`}>
            ...
            <div className={`flex items-center ${collapsed ? "" : "gap-2.5"}`}>
              ...
              {!collapsed && (
                <div className="min-w-0">
                  ...
                  <p className="truncate text-[12px] text-[#596382]">{session.email}</p>
                </div>
              )}
            </div>
            {!collapsed && <p className="mt-3 border-t border-[#1b2340] pt-2 text-[11px] uppercase tracking-[.12em] text-[#3e4762]">{session.role === "admin" ? "Administrador" : "Acceso de inversionista"}</p>}
          </div>
        </div>
      </aside>
