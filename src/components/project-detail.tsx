  ...
  return (
    <>
      ...
      <section className="panel mt-4 overflow-hidden">
        <div className="border-b border-[#202b25] p-5"><p className="text-[13px] font-semibold">Project documents</p><p className="mt-1 text-[10px] text-[#748078]">Reports, receipts, and construction updates</p></div>
        <div className="divide-y divide-[#202b25]">
          {visibleDocuments.slice(0, 5).map((document) => document.fileUrl !== "#" ? (
            <a key={document.id} href={document.fileUrl} className="table-row group grid w-full grid-cols-[40px_minmax(0,1fr)_36px] items-center gap-3 px-4 py-3.5 sm:px-5" aria-label={`Open ${document.title}`}>
              ...
              <span className="min-w-0"><span className="block truncate text-[11px] font-medium">{document.title}</span><span className="mt-1 block text-[9px] capitalize text-[#718078]">{document.type} · {document.uploadedDate}</span></span>
            ...
            <div key={document.id} className="grid w-full grid-cols-[40px_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3.5 sm:px-5">
              <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-[#211d3b] text-[10px] font-semibold uppercase text-[#aa99ff]">{document.type.slice(0, 1)}</span>
              <span className="min-w-0"><span className="block truncate text-[11px] font-medium">{document.title}</span><span className="mt-1 block text-[9px] capitalize text-[#718078]">{document.type} · {document.uploadedDate}</span></span>
              <span className="rounded-md bg-[#2a2115] px-2 py-1 text-[8px] text-[#d6aa57]">File pending</span>
            </div>
          ))}
        </div>
        {visibleDocuments.length === 0 ? <div className="p-10 text-center text-[11px] text-[#718078]">No documents are available yet.</div> : null}
      </section>
