...
export function ProjectDetail({ data, project, session }: { data: DashboardData; project: Project; session: Session }) {
  ...
  return (
    <>
      <div className="mb-5 flex items-center gap-1.5 text-[10px] text-[#68766e]"><Link href={session.role === "admin" ? "/admin" : "/dashboard"}>Portafolio</Link><ChevronRight size={11} /><span className="text-[#a3b0a8]">{project.name}</span></div>
      ...
      <section className="panel mt-6 grid divide-y divide-[#202b25] sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-5">{stats.map(([label, value]) => <div key={label} className="p-4"><p className="text-[9px] text-[#7d8a82]">{label}</p><p className="tabular mt-2 text-[16px] font-semibold">{value}</p></div>)}</section>
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.45fr_1fr]">
        <article className="panel p-5"><div className="flex items-start justify-between"><div><p className="text-[13px] font-semibold">Historial de distribuciones</p><p className="mt-1 text-[10px] text-[#7d8a82]">{investorStake ? "Pagos recibidos por tu participación" : "Pagos de todos los inversionistas del proyecto"}</p></div><p className="tabular text-[13px] font-semibold text-[#4fe8b8]">{compactMoney.format(distributed)}</p></div><div className="mt-5 h-[250px]">{distributionValues.length ? <DistributionLineChart values={distributionValues} labels={distributionLabels} /> : <div className="grid h-full place-items-center text-[10px] text-[#718078]">Todavía no hay distribuciones.</div>}</div></article>
        <article className="panel p-5"><p className="text-[13px] font-semibold">Distribución del presupuesto</p><p className="mt-1 text-[10px] text-[#7d8a82]">Asignación por categoría</p><div className="mt-6"><BudgetDonutChart items={project.budgetBreakdown} /></div></article>
      </section>
      <section className="mt-4 grid gap-4 xl:grid-cols-[1fr_1.4fr]">
        <article className="panel p-5"><p className="text-[13px] font-semibold">Línea de tiempo</p><p className="mt-1 text-[10px] text-[#7d8a82]">Plan y avance del proyecto</p>{project.milestones.length ? <div className="mt-6 space-y-0">{project.milestones.map((milestone, index) => <div key={`${milestone.label}-${index}`} className="relative flex gap-3 pb-6 last:pb-0"><div className={`relative z-10 mt-0.5 size-3 rounded-full ring-4 ring-[#141c18] ${milestone.complete ? "bg-[#4fe8b8]" : "bg-[#6c5ce7]"}`} />{index < project.milestones.length - 1 ? <div className="absolute left-[5px] top-3 h-full w-px bg-[#26332c]" /> : null}<div><p className="text-[11px] font-medium">{milestone.label}</p><p className="mt-1 text-[9px] text-[#74827a]">{milestone.detail}</p></div></div>)}</div> : <div className="grid min-h-44 place-items-center text-center text-[10px] text-[#718078]">Agrega los hitos desde “Editar este proyecto”.</div>}</article>
        <article className="panel overflow-hidden"><div className="flex items-center justify-between border-b border-[#202b25] p-5"><div><p className="text-[13px] font-semibold">{session.role === "admin" ? "Inversionistas del proyecto" : "Tu participación"}</p><p className="mt-1 text-[10px] text-[#7d8a82]">{visibleStakes.length} {visibleStakes.length === 1 ? "participación" : "inversionistas"}</p></div></div><div className="overflow-x-auto"><table className="w-full min-w-[580px] text-left"><thead className="bg-[#141c18] text-[9px] uppercase tracking-wider text-[#5f6c64]"><tr><th className="px-5 py-3 font-medium">Inversionista</th><th className="px-4 py-3 text-right font-medium">Participación</th><th className="px-4 py-3 text-right font-medium">Capital</th><th className="px-5 py-3 text-right font-medium">Pagado a la fecha</th></tr></thead><tbody>{visibleStakes.map((stake) => { const investor = data.investors.find((item) => item.id === stake.investorId)!; return <tr key={stake.investorId} className="table-row border-t border-[#202b25]/70 text-[11px]"><td className="px-5 py-3.5"><div className="flex items-center gap-2.5"><span className="grid size-7 place-items-center rounded-lg bg-[#211d3b] text-[9px] text-[#aa99ff]">{investor.name.split(" ").map((word) => word[0]).join("")}</span><span>{investor.name}</span></div></td><td className="tabular px-4 py-3.5 text-right">{stake.stakePct}%</td><td className="tabular px-4 py-3.5 text-right">{compactMoney.format(capitalForStake(stake))}</td><td className="tabular px-5 py-3.5 text-right text-[#4fe8b8]">{compactMoney.format(distributionsForStake(data, stake))}</td></tr>; })}</tbody></table></div>{visibleStakes.length === 0 ? <div className="p-10 text-center text-[11px] text-[#7d8a82]">Todavía no hay participaciones en este proyecto.</div> : null}</article>
      </section>
