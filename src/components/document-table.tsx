  ...
  return (
    <details className="border-b border-[#232b45] last:border-0">
      <summary className="grid cursor-pointer list-none items-center gap-3 px-4 py-3 sm:grid-cols-[minmax(0,1.5fr)_1fr_1fr_90px_28px]">
        <div className="flex min-w-0 items-center gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[#1b2547] text-[#8fb0ff]"><Icon size={14} /></span><div className="min-w-0"><p className="truncate text-[10px] font-medium">{row.title}</p><p className="mt-1 text-[8px] text-[#596382]">{typeLabels[row.type]}</p></div></div>
        <p className="truncate text-[9px] text-[#8a93b2]">{row.project}</p><p className="truncate text-[9px] text-[#7c86a6]">{row.investor}</p><p className="tabular text-[8px] text-[#596382]">{row.date}</p><PencilLine size={13} className="text-[#6d8dfa]" />
      </summary>
      <div className="border-t border-[#182038] bg-[#0e1425] p-4">
        <form action={updateAction} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5 xl:items-end">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 sm:col-span-2 xl:col-span-5"><div className="flex items-center gap-2">{row.url !== "#" ? <a href={row.url} className="secondary-button px-3"><Download size={12} />Abrir archivo actual</a> : <span className="text-[9px] text-[#6b7594]">Carga un reemplazo para activar el archivo.</span>}<FormMessage state={updateState} /></div><SubmitButton pendingLabel="Actualizando…">Guardar documento</SubmitButton></div>
        </form>
...
export function DocumentTable({ rows, data, editable = false }: { rows: DocumentRow[]; data?: DashboardData; editable?: boolean }) {
  ...
  return (
    <section className="panel overflow-hidden">
      ...
      {visible.length === 0 ? <div className="p-12 text-center text-[11px] text-[#6b7594]">No hay documentos que coincidan con estos filtros.</div> : null}
      <div className="flex items-center justify-between border-t border-[#232b45] px-5 py-3"><p className="text-[9px] text-[#596382]">Mostrando {visible.length} de {filtered.length} documentos</p><div className="flex items-center gap-2"><button aria-label="Página anterior" disabled={currentPage <= 1} onClick={() => setPage((value) => value - 1)} className="grid size-7 place-items-center rounded-lg bg-[#1a2340] disabled:opacity-30"><ChevronLeft size={13} /></button><span className="text-[9px] text-[#7c86a6]">{currentPage} / {pages}</span><button aria-label="Página siguiente" disabled={currentPage >= pages} onClick={() => setPage((value) => value + 1)} className="grid size-7 place-items-center rounded-lg bg-[#1a2340] disabled:opacity-30"><ChevronRight size={13} /></button></div></div>
    </section>
  );
}
