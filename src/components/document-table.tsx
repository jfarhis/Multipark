  ...
  return (
    <details className="border-b border-[#202b25] last:border-0">
      <summary className="grid cursor-pointer list-none items-center gap-3 px-4 py-3 sm:grid-cols-[minmax(0,1.5fr)_1fr_1fr_90px_28px]">
        <div className="flex min-w-0 items-center gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[#211d3b] text-[#aa99ff]"><Icon size={14} /></span><div className="min-w-0"><p className="truncate text-[10px] font-medium">{row.title}</p><p className="mt-1 text-[8px] text-[#657269]">{typeLabels[row.type]}</p></div></div>
      ...
      <div className="border-t border-[#1a241e] bg-[#0b110e] p-4">
        <form action={updateAction} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5 xl:items-end">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 sm:col-span-2 xl:col-span-5"><div className="flex items-center gap-2">{row.url !== "#" ? <a href={row.url} className="secondary-button px-3"><Download size={12} />Abrir archivo actual</a> : <span className="text-[9px] text-[#68766e]">Carga un reemplazo para activar el archivo.</span>}<FormMessage state={updateState} /></div><SubmitButton pendingLabel="Actualizando…">Guardar documento</SubmitButton></div>
        </form>
