...
export function DocumentUploadForm({ data }: { data: DashboardData }) {
  ...
  return (
    <section className="panel mb-4 p-5">
      <div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#ffc46b]/8 text-[#ffc46b]"><FileUp size={16} /></span><div><p className="text-[12px] font-semibold">Carga segura de documentos</p><p className="mt-1 text-[10px] text-[#a3806f]">Los archivos son privados y se entregan únicamente después de verificar los permisos.</p></div></div>
      ...
      </form> : <div className="mt-5 rounded-xl border border-[#402620] bg-[#1f100c] p-4 text-[10px] text-[#a3806f]">Primero crea un proyecto para poder agregar documentos.</div>}
    </section>
  );
}
