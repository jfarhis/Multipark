  ...
  return (
    <details className="border-b border-[#35201a] last:border-0">
      <summary className="grid cursor-pointer list-none items-center gap-3 px-4 py-3 sm:grid-cols-[1.2fr_1fr_110px_110px_28px]">
        <div className="min-w-0"><p className="truncate text-[10px] font-medium">{investor?.name ?? distribution.investorId}</p><p className="mt-1 truncate text-[8px] text-[#82604e]">{distribution.id}</p></div>
        <p className="truncate text-[9px] text-[#b18f7e]">{project?.name ?? distribution.projectId}</p>
        <p className="tabular text-[9px] text-[#a3806f]">{distribution.date}</p>
        <p className="tabular text-[10px] font-semibold text-[#ffc46b] sm:text-right">{money.format(distribution.amount)}</p>
        <PencilLine size={13} className="text-[#e07a2e]" />
      </summary>
  ...
  return (
    <>
      <section className="panel p-5">
        <div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#ff6b3d]/10 text-[#ffc46b]"><Plus size={15} /></span><div><p className="text-[12px] font-semibold">Registrar una distribución</p><p className="mt-1 text-[9px] text-[#96725f]">Agrega el pago al panel del inversionista y guarda opcionalmente su comprobante privado.</p></div></div>
        ...
        </form> : <div className="mt-5 rounded-xl border border-[#402620] bg-[#1f100c] p-4 text-[10px] leading-5 text-[#a3806f]">Primero crea un proyecto, agrega un inversionista y asígnale una participación.</div>}
      </section>
      <section className="panel mt-4 overflow-hidden">
        <div className="border-b border-[#35201a] px-4 py-3"><p className="text-[11px] font-semibold">Historial de distribuciones</p><p className="mt-1 text-[8px] text-[#82604e]">Selecciona una fila para editar, reemplazar el comprobante o eliminarla.</p></div>
        <div>{sorted.map((distribution) => <DistributionEditor key={distribution.id} distribution={distribution} data={data} />)}</div>
        {sorted.length === 0 ? <div className="p-10 text-center text-[10px] text-[#96725f]">Todavía no hay distribuciones registradas.</div> : null}
      </section>
    </>
  );
}
