  ...
  return (
    <details className="panel mt-5 overflow-hidden">
      <summary className="flex cursor-pointer list-none items-center gap-3 p-4 text-[11px] font-semibold">
        ...
        <span className="ml-auto text-[9px] font-normal text-[#6b7594]">Controles de administrador</span>
      </summary>
      <div className="border-t border-[#232b45] p-5">
        <form action={updateAction} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="border-t border-[#232b45] pt-5 sm:col-span-2 xl:col-span-4"><p className="text-[11px] font-semibold">Línea de tiempo editable</p><p className="mt-1 text-[9px] text-[#6b7594]">Agrega hasta seis hitos. Deja el nombre vacío para ocultar una fila.</p></div>
          <div className="grid gap-3 sm:col-span-2 xl:col-span-4 xl:grid-cols-2">
            {milestoneRows.map((milestone, index) => <div key={index} className="grid gap-3 rounded-xl border border-[#232b45] bg-[#0e1425] p-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end"><label className="field-label">Hito {index + 1}<input name={`milestoneLabel${index}`} defaultValue={milestone.label} className="input-shell mt-2 h-10 px-3" placeholder="Ej. Adquisición del terreno" /></label><label className="field-label">Detalle o fecha<input name={`milestoneDetail${index}`} defaultValue={milestone.detail} className="input-shell mt-2 h-10 px-3" placeholder="Ej. Terminado o marzo 2027" /></label><label className="flex h-10 items-center gap-2 text-[9px] text-[#8a93b2]"><input name={`milestoneComplete${index}`} type="checkbox" defaultChecked={milestone.complete} className="size-4 accent-[#4d7cfe]" />Terminado</label></div>)}
          </div>
        ...
        <div className="mt-6 border-t border-[#342126] pt-5">
          ...
          <p className="mt-1 text-[9px] leading-4 text-[#7a7f9e]">También elimina sus participaciones, distribuciones y documentos. Escribe exactamente el nombre del proyecto para confirmar.</p>
